...
const stdin = () => {
  ...
  let len = innerData[0]
  return String.fromCharCode(...innerData.slice(2,len+2))
}

async function waitPyodideReady() {
  pyodide = await loadPyodide({ stderr, stdout, stdin });
  postmessage("idle", []);
}
...
// in order to make sure that vite doesn't pack this down into a data url, we flood garbage in the comment
